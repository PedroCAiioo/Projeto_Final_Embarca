...
int main() {
    ...
    const uint PIN = 3;  // GPIO3
  
    gpio_init(PIN);
    gpio_set_dir(PIN, GPIO_OUT);// defini GPIO3 como saida
    ...
    while (true) {
        ...
        // Condicao para ligar o sistema de controle de temperatura
        if(average >= 36.00)
        {gpio_put(PIN, 1);// Seta o pino em nível lógico alto (3,3V)

          sleep_ms(5000); //   Delay para caso haja uma variaçao em torno do criterio de acionamento,
        }                 // para a ventoinha nao ficar ligando e deligando.
         
          else{gpio_put(PIN, 0);}  // Seta o pino em nível lógico alto (0V)
